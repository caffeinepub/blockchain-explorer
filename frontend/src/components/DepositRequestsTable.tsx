...
import { useApproveDepositRequest, useRejectDepositRequest } from '../hooks/useAdminQueries';
...
export default function DepositRequestsTable({ requests }: DepositRequestsTableProps) {
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border hover:bg-transparent">
            ...
            <TableHead className="text-muted-foreground text-xs">UTR</TableHead>
          ...
          {requests.map((req) => {
            ...
            return (
              <TableRow key={req.id.toString()} className="border-border hover:bg-matkaDark-raised/50">
                ...
                <TableCell className="text-right font-bold text-sm text-matkaGold">
                  {formatPaiseToINR(req.amount)}
                </TableCell>
                <TableCell className="text-xs font-mono">{req.utr}</TableCell>
